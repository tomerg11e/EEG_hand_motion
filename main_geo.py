...
from EEGDataset import EEGDenseDataset
...
def get_data_geo(data_path: str, train_ration: tuple, arguments, upper_cap=None):
    dataset = EEGDenseDataset(None, data_path, None, upper_cap=upper_cap)
...
def train_lightning_model(model_name, model, train_loader, val_loader, test_loader, max_epochs, wandb_upload, **kwargs):
    ...
    if wandb_upload:
        ...
        run_name = f"static_GNN, {date_time}"
    ...
    result = {"test": test_result[0]['acc/test'], "train": train_result[0]['acc/test']}
    return model, result
